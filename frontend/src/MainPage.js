...
function MainPage () { 
    const [task, setTask] = useState('');
      ...
      async function fetchtime() {  
        const surah = await fetch('https://api.alquran.cloud/v1/surah/2') ;
        const time = await fetch('https://api.aladhan.com/v1/timingsByCity?city=Sfax&country=Tunisia&method=3&school=0&latitudeAdjustmentMethod=3&timezonestring=Africa/Tunis&tune=2,1,0,0,1,0,0,5');
    const surahdata=await surah.json() ; 
        const data=await time.json() ;
        setTime(data.data.timings) ; 
        console.log(surahdata) ;   
      const  ayat =  surahdata.data.ayahs 
      for (let i = 0; i < ayat.length; i++) { 
        console.log(ayat[i].text) ; 
